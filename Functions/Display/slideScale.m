% This function has been adapted from https://github.com/JAQuent/functions-for-matlab/blob/master/slideScale.m
% This function can now take a number between 0-100 as a start position for
% starting the slider based on previous position
...
%    'startposition'  -> Choose 'right', 'left', 'center' or a number between 0-100 as start position.
%                        Default is center.
...
%                        scale. 0 is top and 1 is bottom. Default is 0.65.
...
%                       variable is 1.
%
%   Author: Joern Alexander Quent & Brendan Williams
%   e-mail: Alex.Quent@mrc-cbu.cam.ac.uk; b.williams3@reading.ac.uk
%   Version history:
...
elseif startPosition >= 0 & startPosition <= 100
    %x = ((globalRect(3)*scalaLength)/100)*startPosition;
    x = globalRect(3)*(1-scalaLength) + ((globalRect(3)*scalaLength-globalRect(3)*(1-scalaLength))/100)*startPosition;
    %x = globalRect(3)*(1-scalaLength) + ((globalRect(3)*scalaLength)/100)*startPosition;
else
    error('Only right, center, left, and numbers between 0 and 100 are possible start positions');
end
